...
with KuavoSim() as bot:
    ...
    for i, (left, right) in enumerate(pts[:8]):
        t0 = time.time()
        print('solve_begin', i, left, right, flush=True)
        res = bot.solve_ik(left, right, frame=2, timeout=3.0)
        print('solve_end', i, 'dt', round(time.time()-t0, 3), 'success', getattr(res, 'success', None), 'err', getattr(res, 'error_reason', None), flush=True)
